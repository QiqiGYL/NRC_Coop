...
- **bounds.csv**: Generated from step 3.

- **big_table.csv**: Generated from step 3.

- **big_table.db**: Convert **big_table.csv** to Sqlite3 database by using Step 5.

- **big_table_128.csv**: Extract `lambda = 128` rows from **big_table.csv**.

- **big_table_128.db**: Convert **big_table_128.csv** to Sqlite3 database by using Step 5.
...
## 1. Generating Random Ciphertexts in C++
...
## 4. Calculate OpenFHE Precision with C++ Code (using CSV)
...
## 5. Convert CSV to SQLite3 Database with Python
...
## 6. Calculate OpenFHE Precision with C++ Code (using SQLite3 Database)
...
## 7. Find The Minimum OpenFHE Precision with Python 
...
## 8. CKKS Parameters Optimization with Python (Version 1)

> **Note:** This version is for advanced users. Regular users can proceed to Step 9.
...
### Prerequisites
Before running this script, ensure the following:
- A SQLite database file named `big_table_128_precision_final1_f.db` must exist in the `openfhe-development/build` directory.  (The user can manually change the imported database file name in `CKKS_parameters_opt_GUI.py`.)
...
3. Execute the script using Python:
   ```bash
   python3 CKKS_parameters_opt_GUI.py
   ```
...
## 11. Benchmarking with Multi CSV File Outputs (version 2)
