...
# filter db to delete the ciphertext levels, only output the minimum openfhe precision

# Input and output database filenames
input_db = "big_table_128_precision_final1.db"  #user can change the input
table_name_in = "big_table_128_precision_final1" #user can change the input table name
...
def process_database(input_db, output_db):
    ...
    SELECT lambda, log2_N, log2_delta, q0, L, log2_precision, CAST(MIN(OpenFHE_precision) AS INTEGER) AS OpenFHE_precision
    FROM input_db.{table_name_in}
    WHERE OpenFHE_precision > 0
    GROUP BY lambda, log2_N, log2_delta, q0, L, log2_precision;
    """
